...
internal final class ServiceProviderImpl: ServiceProvider {
    private let types: [String: [(Configuration, ServiceProvider) async throws -> Any]];
    private let configuration: Configuration;
    private var builtTypes: [String: [Any]];

    internal init(configuration: Configuration, types: [String: [(Configuration, ServiceProvider) async throws -> Any]]) {
    ...
    private func getCollection<T>(_ name: String) async throws -> [T] {
        ...
        if let instanceBuilders = types[key] {
            for instanceBuilder in instanceBuilders {
                let instance = try await instanceBuilder(configuration, self);
